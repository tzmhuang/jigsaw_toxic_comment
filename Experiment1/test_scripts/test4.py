...
FILE_NAME = ENCODE_DIR + \
    '\\{0}_encoding.npy'.format(MODEL.replace('/', '-'))
...
x_train, y_train, x_validate, y_validate,x_validate_2, y_validate_2, x_test = encoding(
    ...
    test_data_path=TEST_DIR, tokenizer=tokenizer, max_len=models.MAX_LEN)
...
model = models.distilbert_hs_mean_max_dense(transformer)
model.compile(tf.keras.optimizers.Adam(5e-6), tf.keras.losses.BinaryCrossentropy(label_smoothing=0.0),
              metrics=[tf.keras.metrics.BinaryAccuracy(), tf.keras.metrics.AUC()])

# tensorboard setup
log_dir = FIT_LOG_DIR + '\\'+TEST_NAME+ \
